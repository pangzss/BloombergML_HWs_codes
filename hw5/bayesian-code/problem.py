...
def likelihood_func(w, X, y_train, likelihood_var):
    ...
    w = w[...,None]
    sigma_2 = likelihood_var
    likelihoods = 1/(np.sqrt(2*np.pi*sigma_2))* \
                np.exp(-( (y_train-X@w)**2 ) / (2*sigma_2) )
    likelihood = 1.
    for temp in likelihoods:
        likelihood *= temp
...
def get_posterior_params(X, y_train, prior, likelihood_var = 0.2**2):
    ...
    post_mean = (X.T@X + likelihood_var*SIGMA.getI()).getI()@X.T@y_train
    post_var = ((1/sigma_2)*X.T@X + SIGMA.getI()).getI()
