...
class RidgeRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, l2_reg=1, step_size=.005,  max_num_epochs = 5000):
        ...
        self.L2Loss = nodes.SquaredL2DistanceNode(a=self.prediction, b=self.y,
                                               node_name="square loss")

        self.L2Penalty = nodes.L2NormPenaltyNode(l2_reg=l2_reg, w=self.w, 
                                                node_name='L2 penalty')
                                                
        self.objective = nodes.SumNode(a=self.L2Loss, b=self.L2Penalty,
                                                node_name='objective')
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.max_num_epochs):
            ...
            for i in range(n_batches):
                ...
                end = num_instances if end > num_instances else end
                batch_ids = shuffle[start:end] 
                obj, grads = self.graph.get_gradients(input_values = {"x": X[batch_ids]},
                                                    outcome_values = {"y": y[batch_ids]})
                epoch_obj_tot += obj
